<?php
/* 쿠폰 기본 관리 함수 */
/* 쿠폰 코드 생성 함수 (xxxx-xxxx-xxxx-xxxx 형식) */
function generate_coupon_code($length = 16) {
...
function process_auto_coupons() {
    ...
    try {
        ...
        // 3. 첫 구매 쿠폰 발급
        $log[] = process_first_purchase_coupons();
...
function validate_coupon_data($data) {
    ...
    if (isset($data['ct_type2']) && $data['ct_type2'] == 2 && $data['ct_discount1'] > 100) {
...
/* 쿠폰 자동 발급 함수 (첫 구매 시) */
function auto_issue_coupon_on_first_purchase($member_id, $order_id) {
    global $DB;

    // 회원의 이전 주문 내역이 있는지 확인
    $DB->where('mt_idx', $member_id);
    $DB->where('idx', $order_id, '!='); // 현재 주문 제외
    $previous_orders = $DB->getValue('order_t', 'COUNT(*)');
    ...
    // 첫 구매 자동 발급 설정된 쿠폰 조회
    $DB->where('ct_auto_issue', 'Y');
    $DB->where('ct_auto_type', 3); // 3 = 첫 구매 시
    $DB->where('ct_show', 'Y');
    $first_purchase_coupons = $DB->get('coupon_t');

    if (empty($first_purchase_coupons)) {
        return [
            'success' => true,
            'message' => '첫 구매 자동 발급 설정된 쿠폰이 없습니다.',
            'issued_count' => 0
        ];
    }
    ...
    foreach ($first_purchase_coupons as $coupon) {
        $result = issue_coupon($coupon['idx'], $member_id, '첫 구매 자동 발급');
        $results[] = $result;
    ...
    return [
        'success' => true,
        'message' => "{$issued_count}개의 첫 구매 쿠폰이 자동 발급되었습니다.",
        'issued_count' => $issued_count,
        'results' => $results
    ];
}
...
/* 첫 구매 쿠폰 자동 발급 처리 함수 */
function process_first_purchase_coupons() {
    global $DB;

    // 첫 구매 쿠폰 조회
    $DB->where('ct_method', 6); // 6 = 첫 구매 발급
    $DB->where('ct_show', 'Y');
    $first_purchase_coupons = $DB->get('coupon_t');

    if (empty($first_purchase_coupons)) {
        return '첫 구매 쿠폰이 설정되어 있지 않습니다.';
    }

    // 어제 첫 구매한 회원 조회 (일 1회 실행 기준)
    $yesterday = date('Y-m-d', strtotime('-1 day'));
    ...
    $sql = "SELECT o.mt_idx 
            ...
            AND (
                SELECT COUNT(*) 
                FROM order_t o2 
                WHERE o2.mt_idx = o.mt_idx 
                AND o2.idx < o.idx
            ) = 0";

    $first_purchase_members = $DB->rawQuery($sql, [$yesterday]);

    if (empty($first_purchase_members)) {
        return '어제 첫 구매한 회원이 없습니다.';
    }

    $issued_count = 0;

    foreach ($first_purchase_members as $member) {
        foreach ($first_purchase_coupons as $coupon) {
            // 이미 발급된 쿠폰인지 확인
            $DB->where('ct_idx', $coupon['idx']);
    ...
    return "첫 구매 쿠폰 {$issued_count}개가 발급되었습니다.";
}

/* 주문 완료 시 첫 구매 쿠폰 발급 함수 */
function check_and_issue_first_purchase_coupon($member_id, $order_id) {
    // 회원만 처리
    if (!$member_id) {
        return [
            'success' => false,
    ...
    // 첫 구매 쿠폰 발급 처리
    return auto_issue_coupon_on_first_purchase($member_id, $order_id);
}
