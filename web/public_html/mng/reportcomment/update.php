...
if ($_POST['act'] == "input" || $_POST['act'] == "update") {
    ...
    if ($search_txt !== '') {
      if ($search_col == "all") {
        $DB->where("( INSTR(a1.mt_id, ?) or INSTR(a2.rvt_content, ?) )", [$search_txt, $search_txt]);
      } else {
    ...
    $DB->where('a1.rt_type', '1');
    $DB->where('a1.rt_del', 'N');
    $DB->orderBy("a1.rt_idx", "desc");

    $select = "
        a1.*, a1.rt_idx as nt_idx, a2.rvt_content, a2.rt_del as reply_del
      ";

    $table = "{$CFG_TBL['report']['default']} a1 
            LEFT JOIN {$CFG_TBL['reply']['default']} a2 
            ...
            if ($list) {
              ...
              ?>
                <tr draggable="false" data-id="<?=$row['rt_idx']?>">
                    ...
                    <td class="text-center">
                        <? if($row['reply_del']=='Y'){?>
                          <span class="line1_text">삭제된 댓글입니다.</span>
                        <? } else {?>
                          <span class="line1_text"><?=$row['rvt_content']?></span>
                        <?}?>
                    </td>
                    <td class="text-center">
...
else if($_POST['act'] == "reply_update") {
  ...
  try {
    ...
    if($_POST['rt_status']=='2'){
      if($reply){
        ...
        $arr_query = ['rt_del' => 'Y'];
        if(!$DB->update($CFG_TBL['reply']['default'], $arr_query)) {
