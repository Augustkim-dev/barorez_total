...
if ($_POST['act'] == "input" || $_POST['act'] == "update") {
    ...
    if ($search_txt !== '') {
      if ($search_col == "all") {
        $DB->where("( INSTR(a1.mt_id, ?) or INSTR(a2.jt_content, ?) )", [$search_txt, $search_txt]);
      } else {
    ...
    $DB->where('a1.rt_type', '2');
    $DB->where('a1.rt_del', 'N');
    $DB->orderBy("a1.rt_idx", "desc");

    $select = "
        a1.*, a1.rt_idx as nt_idx, a2.jt_content, a2.jt_del as join_del
      ";

    $table = "{$CFG_TBL['report']['default']} a1 
            LEFT JOIN {$CFG_TBL['join']['default']} a2 
            ON a1.rt_pidx = a2.jt_idx";
            ...
            if ($list) {
              ...
              ?>
                <tr draggable="false" data-id="<?=$row['jt_idx']?>">
                    ...
                    <td class="text-center">
                        <? if($row['join_del']=='Y'){?>
                          <span class="line1_text">삭제된 조인입니다.</span>
                        <? } else {?>
...
<?php
  ...
  try {
    ...
    if($_POST['rt_status']=='2'){
      if($join){
        $DB->where('jt_idx', $join['jt_idx']);
        $arr_query = ['jt_del' => 'Y'];
        if(!$DB->update($CFG_TBL['join']['default'], $arr_query)) {
          throw new Exception("조인 삭제 실패했습니다.");
        }
      }
    } else {
      ...
      if($join){
        ...
        $arr_query = ['jt_del' => 'N'];
        if(!$DB->update($CFG_TBL['join']['default'], $arr_query)) {
          throw new Exception("조인복구 실패했습니다.");
        }
      }
