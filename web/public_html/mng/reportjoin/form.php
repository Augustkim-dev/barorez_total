...
if ($_GET['act'] == "view") {
    ...
    $DB->where('jt_idx', $row['rt_pidx']);
    $DB->where('jt_del', 'N');
    $join = $DB->getone($CFG_TBL['join']['default']);

    $DB->where('gmt_idx', $row['gmt_idx']);
    $DB->where('rt_del', 'N');
    $review = $DB->getone($CFG_TBL['review']['default'], ' avg(rt_average_start) as avg ');
    ...
    $join_mt_id = $join['mt_id'];
    $join_jt_wdate = DateType($join['jt_wdate'], 4);
    $join_jt_jdate = DateType($join['jt_jdate'], 4);
    $join_jt_content = $join['jt_content'];
    $join_review_avg = $review['avg'] ?? 0;
    $join_gmt_golf_name = $join['gmt_golf_name'];
    $join_jt_person = $join['jt_person'];
    $join_jt_gender = $arr_jt_gender[$join['jt_gender']];

    if($join['jt_del']=='Y'){
      $join_mt_id = '-';
      $join_jt_wdate = '-';
      $join_jt_jdate = '-';
      $join_jt_content = '삭제된 조인입니다.';
      $join_review_avg = 0;
      $join_gmt_golf_name = '-';
      $join_jt_person = '-';
      $join_jt_gender = '-';
                        ...
                        <div class="tab-content margin-top-15">
                          <div class="tab-pane fade show active" id="step-1" role="tabpanel" aria-labelledby="step-tab-1">
                            ...
                            <div class="form-group row margin-top-30">
                              ...
                              <div class="col-sm-4">
                                <input type="text" name="mt_id" id="mt_id" value="<?=$join_mt_id?>" class="form-control" <?=$isDsiabled?> />
                              </div>
                              <label for="jt_wdate" class="col-sm-2 col-form-label">작성일</label>
                              <div class="col-sm-4">
                                <input type="text" name="jt_wdate" id="jt_wdate" value="<?=$join_jt_wdate?>" class="form-control" <?=$isDsiabled?> />
                              </div>
                            </div>
                            <div class="form-group row">
                              <label class="col-sm-2 col-form-label">제목</label>
                              <div class="col-sm-10">
                                <input type="text" value="<?=$join_jt_content?>" class="form-control" <?=$isDsiabled?> />
                              </div>
                            </div>
                            <div class="form-group row">
                              <label for="jt_content" class="col-sm-2 col-form-label">내용</label>
                              <div class="col-sm-10">
                                <div>
                                  <div class="d-flex mb-2">
                                    <?=$join_gmt_golf_name?> /
                                    <?=renderStars($join_review_avg)?>
                                  </div>
                                  <div class="d-flex mb-2">
                                    <?=$join_jt_jdate?>
                                    초청인원 : <?=$join_jt_gender?> / <?=$join_jt_person?>명
                                  </div>
