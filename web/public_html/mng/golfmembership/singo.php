      ...
      <div class="card margin-bottom-0">
        <div class="card-body">
          ...
          <div class="form-row">
            ...
            <div class="col-12">
              <form method="POST" name="frm_search" id="frm_search" action="<?=$_SERVER['PHP_SELF']?>?nt_idx=<?php echo $_GET['nt_idx']?>" onsubmit="return frm_search_chk(this, event);" class="d-flex justify-content-end align-items-center">
                ...
                <select class="form-control searchByStatus" name="sel_search" id="sel_search" tabindex="-1" aria-hidden="true">
                  ...
                  <option value="a1.reason_text">내용</option>
                ...
                <button type="button" class="btn btn-gray" onclick="location.href='./singo.php?nt_idx=<?php echo $_GET['nt_idx']?>'">초기화</button>
              </form>
            </div>
          </div>
          ...
          <form name="frm_list" id="frm_list" onsubmit="return false;">
            <input type="hidden" name="act" id="act" value="singo_list" />
            ...
            <input type="hidden" name="obj_nt_idx" id="obj_nt_idx" value="<?=$_GET['nt_idx']?>" />
          </form>
          ...
          <script>
              ...
              function updateToggleStatus(url, data, $element) {
                  $.ajax({
                      ...
                      success: function(response) {
                          ...
                          if(response.success) {
                              toastr.success('처리여부 상태가 변경되었습니다.');
                          } else {
              ...
              $(document).ready(function() {
                  ...
                  // 카테고리 토글 이벤트 초기화
                  initToggleHandler(
                      '.switch input[name="is_processed"]',  // 셀렉터
                      'id',                             // data 속성 이름
                      './update.php'    // 업데이트 URL
                  );
              });
