...
if ($_POST['act'] == "input") {
  ...
  try {
    ...
    // tp_order 최대값 조회
    $max_order = $DB->getValue($CFG_TBL['faq']['default'], "COALESCE(MAX(nt_order), 0) + 1");
    ...
    $_last_idx = $DB->insert($CFG_TBL['faq']['default'], $arr_query);

//    if($_last_idx) {
//      for($i = 1; $i <= $_POST['file_count']; $i++) {
//        if($_FILES["nt_file{$i}"]['name']) {
//          handle_file_upload($_POST['board'], $_FILES["nt_file{$i}"], $i, $_last_idx, "nt_file{$i}", $ct_faq_dir);
//        }
//      }
//    }
...
//      $tpl_sql = "SELECT * FROM fcm_template_t WHERE type = 'faq' AND activity = 'input' LIMIT 1";
...
//    }
  ...
  try {
    ...
    $DB->update($CFG_TBL['faq']['default'], $arr_query);
  ...
  try {
    ...
    $row = $DB->getOne($CFG_TBL['faq']['default']);
...
//    $DB->update($CFG_TBL['faq']['default'], $arr_query);
  ...
  $DB->join($CFG_TBL['category']['faq']." a2", "a1.ct_idx = a2.idx", "LEFT");
  ...
  $list = $DB->arraybuilder()->paginate($CFG_TBL['faq']['default']." a1", $pg, '*, a2.ct_title as category, a1.idx as nt_idx');
...
<!--            </td>-->
            ...
            <td data-title="관리" class="text-center">
              ...
              <td data-title="FAQ 카테고리">
                  <span class="line1_text"><?=$row['category']?></span>
              </td>
            <td data-title="제목">
            ...
            <td data-title="등록일시" class="text-center">
                <span>등록일: <?=DateType($row['nt_wdate'], 4)?></span>
                <span>수정일: <?=DateType($row['nt_udate'], 4)?></span>
            </td>
          </tr>
          <?php
          $counts--;
  ...
  if($n_page > 1) {
    ...
    $result = $DB->update('faq_t', $arr_query);
