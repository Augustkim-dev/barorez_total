...
if ($_POST['act'] == "input" || $_POST['act'] == "update") {
    ...
    try {
        ...
        if($_POST['act'] == "update"){
          ...
          $old_data = $DB->getOne($CFG_TBL['review']['default']);
          if(!$old_data) {
            throw new Exception("데이터가 없습니다.");
          }
        }

        $max_order = $DB->getValue($CFG_TBL['review']['default'], "COALESCE(MAX(rt_order), 0) + 1");
        ...
        if($_POST['act'] == "update"){
          ...
          $_last_idx = $DB->insert($CFG_TBL['review']['default'], $arr_query);
          if(!$_last_idx) {
        ...
        foreach ($_FILES as $key => $file) {
          if ($file['error'] === 0) {
            ...
            if (move_uploaded_file($file['tmp_name'], $filepath)) {
              ...
              $DB->where('rt_idx', $_last_idx);
              if (!$DB->update($CFG_TBL['review']['default'], $update_data)) {
    ...
    try {
        ...
        $row = $DB->getOne($CFG_TBL['review']['default']);
        ...
        $arr_query = ['rt_del' =>'Y'];
        $DB->update($CFG_TBL['review']['default'], $arr_query);
            ...
            if ($list) {
              ...
              ?>
                <tr draggable="false" data-id="<?=$row['rt_idx']?>">
                    ...
                    <td class="text-center">
                      <span class="line1_text"><?=$row['rt_average_start']?></span>
                    </td>

                    <td class="text-center">
                      <span class="line1_text"><?=$row['like_count']?></span>
                    </td>
                    <td  class="text-center">
...
<?php
  ...
  $DB->where('rt_idx', $_POST['id']);
  $arr_query = array(
    'rt_show' => $ct_show
  );
  $result = $DB->update($CFG_TBL['review']['default'], $arr_query);
