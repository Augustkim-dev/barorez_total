...
if ($_POST['act'] == "input") {
  ...
  try {
    ...
    $DB->update($CFG_TBL['board']['default'], $arr_query);
    ...
    for($i = 1; $i <= $_POST['file_count']; $i++) {
      if($_POST["file{$i}_delete"] == 'Y') {
        ...
        $DB->where('bf_no', $i);
        $old_file = $DB->getOne($CFG_TBL['board']['file']);

        if($old_file) {
          @unlink($ct_notice_dir.$old_file['bf_file']);
  ...
  header('Content-Type: application/json');
    try {
        ...
        $row = $DB->getOne($CFG_TBL['board']['default']);
    ...
    $DB->setTrace(true);
    $DB->where('idx', $_POST['idx_t']);
    $row_mt = $DB->getone('board_t', "*, idx as mt_idx");
...
elseif ($_POST['act'] == "list") {
  ...
  if ($_POST['obj_search_txt']) {
    if ($_POST['obj_sel_search'] == "all") {
      $DB->where('( instr(a1.nt_title, \''.$_POST['obj_search_txt'].'\') or instr(a1.mt_id, \''.$_POST['obj_search_txt'].'\') or instr(a1.mt_name, \''.$_POST['obj_search_txt'].'\'))');
    }
        ...
        $DB->where('( instr(a1.nt_title, \''.$_POST['obj_search_txt'].'\'))');
    }
    else {
  ...
  if($_POST['obj_sel_category'] != ''){
      $DB->where('nt_category', $_POST['obj_sel_category']);
  }
  ...
  if($_POST['type'] === 'history'){
      if ($_POST['obj_search_day']) {
          ...
          if ($_POST['obj_search_day'] == '1') {
              // 오늘 00:00:00부터
              $DB->where('a1.del_date', $today . ' 00:00:00', '>=');

          } elseif ($_POST['obj_search_day'] == '2') {
              // 최근 7일
              $start_date = date('Y-m-d', strtotime('-7 days'));
              $DB->where('a1.del_date', $start_date . ' 00:00:00', '>=');
      ...
      if ($_POST['sdate'] && $_POST['edate']) {
          ...
          $DB->where('a1.del_date', [$start, $end], 'BETWEEN');
      }
      $DB->where('a1.del_date');
  }else{
      if ($_POST['obj_search_day']) {
          $today = date('Y-m-d'); // 오늘 날짜 (시간 제외)
          ...
          if ($_POST['obj_search_day'] == '1') {
              // 오늘 00:00:00부터
              $DB->where('a1.nt_wdate', $today . ' 00:00:00', '>=');

          } elseif ($_POST['obj_search_day'] == '2') {
              // 최근 7일
              $start_date = date('Y-m-d', strtotime('-7 days'));
              $DB->where('a1.nt_wdate', $start_date . ' 00:00:00', '>=');
      ...
      if ($_POST['sdate'] && $_POST['edate']) {
          ...
          $DB->where('a1.nt_wdate', [$start, $end], 'BETWEEN');
      }
  ...
  $list = $DB->arraybuilder()->paginate($CFG_TBL['board']['default']." a1", $pg, '*, idx as nt_idx');
...
<!--            </td>-->
            ...
            <td data-title="관리" class="text-center">
              <input type="button" class="btn btn-outline-info btn-sm" value="상세" onclick="location.href='./form.php?act=update&nt_idx=<?=$row['nt_idx'].$type?>'" />
              ...
              <td data-title="구분">
                  <span class="line1_text"><?=$row['nt_category']?></span>
              </td>
            <td data-title="제목">
