...
if ($_POST['act'] == "input") {
    ...
    try {
        ...
        $max_order = $DB->getValue($CFG_TBL['popup']['default'], "COALESCE(MAX(rt_order), 0) + 1");
        ...
        $arr_query = array(
          ...
          "rt_link_url" => $_POST['rt_link_url'],
          "rt_link_target" => $_POST['rt_link_target'],
          "rt_order" => $max_order,
        );

        $_last_idx = $DB->insert($CFG_TBL['popup']['default'], $arr_query);
        if(!$_last_idx) {
        ...
        if (!is_dir($ct_popup_dir)) {
          if (!mkdir($ct_popup_dir, 0707, true)) {
            throw new Exception("category 디렉토리 생성 실패");
          }
          chmod($ct_popup_dir, 0707);
        }
        ...
        if (!empty($_FILES)) {
          ...
          foreach ($_FILES as $key => $file) {
            if ($file['error'] === 0 && $filePosition <= $maxFiles) {
              ...
              $filepath = $ct_popup_dir . $filename;
              ...
              if (move_uploaded_file($file['tmp_name'], $filepath)) {
                ...
                if (!$DB->update($CFG_TBL['popup']['default'], $update_data)) {
  ...
  header('Content-Type: application/json');
    ...
    try {
        ...
        $old_data = $DB->getOne($CFG_TBL['popup']['default']);
        ...
        if (!empty($_POST['removed_files'])) {
          ...
          foreach ($removedFiles as $fileNum) {
            if (!empty($finalImages[$fileNum])) {
              $old_file = $ct_popup_dir . $finalImages[$fileNum];
        ...
        $DB->where('idx', $_POST['nt_idx']);
        if(!$DB->update($CFG_TBL['popup']['default'], $arr_query)) {
          ...
          foreach ($_FILES as $key => $file) {
            if ($file['error'] === 0) {
              ...
              if (!empty($finalImages[$position])) {
                $old_file = $ct_popup_dir . $finalImages[$position];
                if(file_exists($old_file)) {
                  unlink($old_file);
                }
              }
        ...
        // 최종 이미지 상태 조회
        $DB->where('idx', $_POST['nt_idx']);
        $final_data = $DB->getOne($CFG_TBL['popup']['default']);
        $finalImages = array(
    ...
    try {
        ...
        $row = $DB->getOne($CFG_TBL['popup']['default']);
        ...
        $DB->where('idx', $_POST['idx']);
        $DB->delete($CFG_TBL['popup']['default']);
    ...
    $list = $DB->arraybuilder()->paginate($CFG_TBL['popup']['default']." a1", $pg, '*, idx as nt_idx');
                ...
                <tr draggable="true" data-id="<?=$row['idx']?>">
                    ...
                    <td class="text-center">
                      ...
                      if(!empty($row['rt_img1'])):
                        $img = resolveImageUrl($row['rt_img1'], $ct_popup_dir, $ct_popup_url);
                        ?>
...
<?php
  ...
  try {
    ...
    foreach ($data as $item) {
      ...
      try {
        ...
        $DB->where('idx', $idx);
        $DB->update($CFG_TBL['popup']['default'], $arr_query);
  ...
  $result = $DB->update($CFG_TBL['popup']['default'], $arr_query);
  ...
  try {
    ...
    for($i = 1; $i <= 5; $i++) {
      ...
      if(!empty($row[$img_key])) {
        $filepath = $ct_popup_dir . $row[$img_key];
