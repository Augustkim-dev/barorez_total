...
try {
    if ($_POST['act'] == "input") {
        ...
        try {
            ...
            if (!is_dir($ct_category_dir)) {
                if (!mkdir($ct_category_dir, 0707, true)) {
                    throw new Exception("category 디렉토리 생성 실패");
                }
                chmod($ct_category_dir, 0707);
            }

            // maxFiles 값 사용
            for ($i = 1; $i <= $maxFiles; $i++) {
                if (isset($_FILES["ct_img{$i}"]) && $_FILES["ct_img{$i}"]['error'] === 0) {
                    $file = $_FILES["ct_img{$i}"];
                    ...
                    $timestamp = time();
                    $filename = "ct_img_".$_last_idx . "_{$i}_{$timestamp}." . $file_ext;
                    $filepath = $ct_category_dir . $filename;

                    if (move_uploaded_file($file['tmp_name'], $filepath)) {
                        chmod($filepath, 0644);

                        // 파일명을 DB에 업데이트
                        $update_data = ["ct_img{$i}" => $filename];
        ...
        try {
            ...
            // 현재 이미지 상태 저장 - 전송된 maxFiles 값 사용
            $finalImages = array();
            for ($i = 1; $i <= $maxFiles; $i++) {
                $finalImages[$i] = !empty($old_data["ct_img{$i}"]) ? $old_data["ct_img{$i}"] : '';
            }
            ...
            if (!empty($_POST['image_order'])) {
                ...
                $finalImages = array_fill(1, $maxFiles, '');
            ...
            if (!empty($_FILES)) {
                foreach ($_FILES as $key => $file) {
                    if ($file['error'] === 0) {
                        // 새 이미지 위치 결정
                        $position = !empty($newPositions) ? array_shift($newPositions) : 2;
            ...
            $finalImages = array(
                ...
                2 => !empty($final_data['ct_img2']) ? $final_data['ct_img2'] : ''
            );
...
if ($_POST['act'] == "list") {
    ...
    $debug = $DB->false; // 모든 쿼리 정보 배열로 반환
    //print_r($debug);
