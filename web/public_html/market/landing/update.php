...
if ($_POST['act'] == "input") {
    ...
    try {
        ...
        $DB->where('rt_gubun', $_POST['rt_gubun']);
        $DB->where('rt_show', 'Y'); // 노출되는 것만 체크
        $count = $DB->getValue($tbl_name, 'count(*)');
        if ($count >= 20) {
          throw new Exception('랜딩이미지는 도메인당 최대 10개까지 등록할 수 있습니다.');
        }
        ...
        if (!is_dir($ct_landing_dir)) {
          if (!mkdir($ct_landing_dir, 0707, true)) {
            throw new Exception("디렉토리 생성 실패");
          }
          chmod($ct_landing_dir, 0707);
        }
          ...
          foreach ($_FILES as $key => $file) {
            if ($file['error'] === 0 && $filePosition <= $maxFiles) {
              ...
              if ($width > $maxWidth || $height > $maxHeight) {
                throw new Exception("이미지 크기가 너무 큽니다. 최대 {$maxWidth}x{$maxHeight}px 까지만 허용됩니다.");
              }

              // 타임스탬프 추가하여 파일명 중복 방지
              $timestamp = time();
              $filename = "rt_img_{$_last_idx}_{$filePosition}_{$timestamp}.{$file_ext}";
              $filepath = $ct_landing_dir . $filename;
        ...
        $gubun = 'A';
        if($_POST['rt_gubun'] == 'portacellar.com'){
          $gubun = 'A';
        } else if($_POST['rt_gubun'] == 'vinporta.com'){
          $gubun = 'B';
        }
  ...
  header('Content-Type: application/json');
    ...
    try {
        ...
        if (!empty($_POST['removed_files'])) {
          ...
          foreach ($removedFiles as $fileNum) {
            if (!empty($finalImages[$fileNum])) {
              $old_file = $ct_landing_dir . $finalImages[$fileNum];
          ...
          foreach ($_FILES as $key => $file) {
            if ($file['error'] === 0) {
              ...
              if (!empty($finalImages[$position])) {
                $old_file = $ct_landing_dir . $finalImages[$position];
                if(file_exists($old_file)) {
                  unlink($old_file);
                }
              }
        ...
        $gubun = 'A';
        if($final_data['rt_gubun'] == 'portacellar.com'){
          $gubun = 'A';
        } else if($final_data['rt_gubun'] == 'vinporta.com'){
          $gubun = 'B';
        }
    ...
    try {
        ...
        $gubun = 'A';
        if($row['gubun'] == 'portacellar.com'){
          $gubun = 'A';
        } else if($row['gubun'] == 'vinporta.com'){
          $gubun = 'B';
        }
    ...
    if($_POST['obj_sel_domain'] == 'A'){
      $DB->where('a1.rt_gubun', 'portacellar.com');
    } else if($_POST['obj_sel_domain'] == 'B'){
      $DB->where('a1.rt_gubun', 'vinporta.com');
    }
            ...
            if ($list) {
                ...
                <tr draggable="true" data-id="<?=$row['idx']?>">
                    ...
                    <td data-title="관리" class="text-center">
                        <input type="button" class="btn btn-outline-info btn-sm" value="수정" onclick="location.href='./form.php?act=update&nt_idx=<?=$row['nt_idx']?>&gubun=<?=$_POST['obj_sel_domain']?>'" />
                    ...
                    <td class="text-center">
                      ...
                      if(!empty($row['rt_img1'])):
                        $img = resolveImageUrl($row['rt_img1'], $ct_landing_dir, $ct_landing_url);
                        ?>
                        <img src="<?php echo $img?>" class="ct_icon" />
                      <?php endif; ?>
                    </td>
                    <td class="text-center">
                      <span class="line1_text"><?=$row['rt_gubun']?></span>
                    </td>
  ...
  try {
    ...
    for($i = 1; $i <= 5; $i++) {
      ...
      if(!empty($row[$img_key])) {
        $filepath = $ct_landing_dir . $row[$img_key];
        if(file_exists($filepath)) {
          $result[$img_key] = array(
            'exists' => true,
            'url' => $ct_landing_url . $row[$img_key],
